...
# ==============================================================================
# NHPP ARRIVAL GENERATION (INVERSION / TIME-TRANSFORMATION METHOD)
# ==============================================================================
def lambda1_rate(t):
...
def plot_arrival_rates():
    ...
    plt.tight_layout()
    plt.savefig(f"{PLOT_DIR}/inversion_arrival_rates.png", dpi=300)
    plt.close()
...
def plot_allocation_heatmap(total_beds, los_dist, coarse_arrivals, fine_arrivals):
    ...
    plt.tight_layout()
    plt.savefig(f"{PLOT_DIR}/inversion_optimal_allocation_heatmap.png", dpi=300)
    plt.close()
...
def plot_los_sensitivity(best_alloc, coarse_arrivals, fine_arrivals):
    ...
    plt.tight_layout()
    plt.savefig(f"{PLOT_DIR}/inversion_los_sensitivity.png", dpi=300)
    plt.close()
...
def plot_capacity_impact(capacities, reps, los_dist):
    ...
    coarse_arrivals = [generate_arrivals_inversion() for _ in range(reps)]
    fine_arrivals = [generate_arrivals_inversion() for _ in range(max(reps * 2, 50))]
    ...
    plt.ylim(bottom=0)
    plt.tight_layout()
    plt.savefig(f"{PLOT_DIR}/inversion_capacity_impact.png", dpi=300)
    plt.close()
...
def main():
    ...
    print("METHOD: TIME-TRANSFORMATION / INVERSION (OPTION A)")
    ...
    coarse_arrivals = [generate_arrivals_inversion() for _ in range(REPLICATIONS)]
    fine_arrivals = [generate_arrivals_inversion() for _ in range(max(REPLICATIONS * 2, 100))]
