...
def estimate_exp_control_variate():
    u = rand.random()
    return math.exp(u) - 1.69 * (
        u - 1 / 2
    )  # Return both the control variate and the original estimate
...
def get_mean_estimate_stratified():
    ...
    for i in range(n_strata):
        ...
        for _ in range(samples_per_stratum):
            u = rand.random() * (1 / n_strata) + (i / n_strata)
            stratum_estimates.append(math.exp(u))
        estimates.append(np.mean(stratum_estimates))
...
def get_mean_estimate_control_variate():
    estimates = [estimate_exp_control_variate() for _ in range(SAMPLES)]
    mean_estimate = np.mean(estimates)
    return mean_estimate
...
def get_confidence_interval_control_variate():
    estimates = [get_mean_estimate_control_variate() for _ in range(10)]
    mean_of_estimates = np.mean(estimates)
    ci_lower, ci_upper = confidence_interval(estimates)
    print(f"Estimated Mean (Control Variate): {mean_of_estimates:.4f}")
    print(f"Estimated variance (Control Variate): {np.var(estimates, ddof=1):.8f}")
    print(
        f"95% Confidence Interval (Control Variate): [{ci_lower:.4f}, {ci_upper:.4f}]"
    )

def get_confidence_interval_stratified():
    estimates = [get_mean_estimate_stratified() for _ in range(10)]
    mean_of_estimates = np.mean(estimates)
    ci_lower, ci_upper = confidence_interval(estimates)
    print(f"Estimated Mean (Stratified): {mean_of_estimates:.4f}")
    print(f"Estimated variance (Stratified): {np.var(estimates, ddof=1):.8f}")
    print(
        f"95% Confidence Interval (Stratified): [{ci_lower:.4f}, {ci_upper:.4f}]"
    )
...
if __name__ == "__main__":
    ...
    get_confidence_interval_control_variate()

    print("\nStratified Sampling Estimation:")
    get_confidence_interval_stratified()