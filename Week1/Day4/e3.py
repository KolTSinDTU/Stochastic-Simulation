...
def estimate_exponential_control_variate():
    # 1. Generate uniform random variable
    u = rand.random()
    # 2. Return control variate adjusted estimate
    return math.exp(u) - 1.69 * (u - 0.5)
...
def get_mean_estimate_control_variate():
    # 1. Generate multiple control variate estimates
    sample_estimates = [estimate_exponential_control_variate() for _ in range(SAMPLES_PER_ESTIMATE)]
    # 2. Return mean
    return np.mean(sample_estimates)
...
def perform_control_variate_estimation():
    # 1. Generate collection of control variate mean estimates
    mean_estimates_list = [get_mean_estimate_control_variate() for _ in range(NUM_ESTIMATES)]
    ...
    # 3. Output results
    print(f"--- Results: Control Variate Estimation ---")
    print(f"Estimated Mean: {overall_mean:.4f}")
    print(f"Estimated Variance: {overall_variance:.8f}")
    print(f"95% Confidence Interval: [{confidence_lower:.4f}, {confidence_upper:.4f}]")
...
if __name__ == "__main__":
    # 1. Run all estimation methods
    perform_crude_estimation()
    print()
    perform_antithetic_estimation()
    print()
    perform_control_variate_estimation()
